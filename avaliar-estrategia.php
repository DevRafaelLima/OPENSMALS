<?php
    ...
    $id = $_GET['cod'];
    $status = $_GET['status_estrategia'];
    $comentario = $_GET['comentarioStatus'];
    ...
    $newEstrategia->setStatus($status);
    $newEstrategia->setComments($comentario);
   
    
    if($EstrategiaDao->avaliar($newEstrategia)){
        header("Location: minha-area.php?avall=success");
    } else {
        header("Location: minha-area.php?avall=error");
    }
